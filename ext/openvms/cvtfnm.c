...
if (CvtToUnix)
    ...
    if (! path)
        {
        path = argv[0];
        if (apl_ConvertFnm (CVT_FNM_UNIX_TO_VMS, path, &CvtFile))
            path = CvtFile;
        }
    printf ("apl_ConvertFnm (\"%s\") = ", path);
    if (apl_ConvertFnm (CVT_FNM_VMS_TO_UNIX, path, &CvtFile))
	path = CvtFile;
    printf ("\"%s\"\n", path);
    }
...
/***                                                                        ***/
/******************************************************************************/
int apl_ConvertFnm (int CvtType, char *FromFileName, char **ToFileName)
{
int status;
