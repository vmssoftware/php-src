...
int rdb_execi(char *stmt)
{
	char * 			tmp;

	errstr[0] = '\0';

	assert(tmp = strdup(stmt));
	eib$$db_exec_immediate(&SQLCA, tmp);
	free(tmp);
...
int rdb_close_cursor(void *addr)
{
	stmt_t * 		sp = (stmt_t *) addr;
...
void *rdb_prepare(char *stmt)
{
	...
	assert(tmp = strdup(stmt));
	eib$$db_prepare(&SQLCA, &sp->id, tmp);
	free(tmp);
...
void *rdb_declare_cursor(char *cursor, char *stmt)
{
	stmt_t * 		sp = NULL;

	errstr[0] = '\0';

	if ((sp = rdb_prepare(stmt)) == NULL)
	{
	   return (NULL);
	}
...
char **rdb_fetch_row(void *addr)
{
	...
	for (i = 0; i < nc; i++)
	{
	   assert(arr[i] = strdup(rdb_data(addr, i)));
	}
